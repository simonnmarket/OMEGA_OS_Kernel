...
| `882d3ec` | DOC-OFC + memorando: alinhamento do intervalo da cadência e tabela de commits |

*Selo temporal de encerramento CEO (secção 8): usar o commit Git mais recente que alterou este ficheiro com a secção 8 presente (`git log -1 -- governance/DOC-OFC-REGISTO-ALINHAMENTO-DOCUMENTAL-SHADOW-PAPER-OIS-20260517.md`). Para a **cadência técnica** de integração Git acima, usar `git show <hash>` por linha; o vértice actual da branch confirma-se com `git rev-parse HEAD`.*
...
Estes itens **não** são pré-requisitos para continuar desenvolvimento na branch de integração após a cadência Git que inicia em `41e75f6` (secção 2; vértice: `git rev-parse HEAD`). São **evidência operacional** ou **governança institucional** com calendário próprio.
...
| **Pacote audit modular + CLI no Git** | **FECHADO** | cadência a partir de `41e75f6` (ver secção 2 + `git rev-parse HEAD`) |
...
1. O pacote encontra-se **implementado e versionado** na branch `feature/nebular-integration-phase1`, incluindo a cadência técnica desde `41e75f6` até o vértice actual (`git rev-parse HEAD`, secção 2) e o memorando PSA em `governance/MEMORANDO_PSA_HANDOVER_OIS-20260517.md`.

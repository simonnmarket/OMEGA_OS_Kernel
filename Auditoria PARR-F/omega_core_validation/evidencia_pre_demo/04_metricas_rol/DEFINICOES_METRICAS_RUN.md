...
Calculado dinamicamente no `ExecutionManagerV104` quando `signal_fired` e ainda não há posição aberta (`active_side is None`), usando o **proxy de spread** alinhado ao código do gateway.
- **Fórmula (implementação):** $|y\_\text{price} - 0{,}5 \cdot x\_\text{price}|$ (equivalente a `abs(y_price - (0.5 * x_price))` em Python).
- **Coluna no CSV:** `opp_cost`
- **Nota:** versões anteriores do MD citavam $|\text{Price}_{\text{signal}} - \text{Price}_{\text{current}}| \times \text{Size}$; essa forma foi **substituída** pela expressão acima para coincidir com `10_mt5_gateway_V10_4_OMNIPRESENT.py`.

...
private:
   ...
   {
      if(!TerminalInfoInteger(TERMINAL_CONNECTED)) { m_logger.log_error("[QREG] Sem conexão com o servidor de mercado"); return false; }
      if(!m_logger.is_initialized()) { m_logger.log_error("[QREG] Logger não inicializado"); return false; }
      return true;
   }
...
public:
   ...
   {
      if(!m_logger.is_initialized()) { Print("[QREG] Logger não inicializado"); ExpertRemove(); }
   ...
   bool InitializeState()
   {
      ...
      int size = (int)MathPow(2, m_qubits); if(size <= 0 || size > 1024) { m_logger.log_error("[QREG] Número de qubits inválido: " + IntegerToString(m_qubits)); return false; }
      ArrayFree(m_data);
      for(int i=0; i<size; i++) { Complex *c = new Complex(0.0, 0.0); if(c == NULL) return false; if(!Add(c)) return false; }
      dynamic_cast<Complex*>(m_data[0])->real = 1.0; m_normalized = true; m_logger.log_info("[QREG] Estado quântico inicializado"); return true;
   }
   ...
   bool ApplyH(int target)
   {
      ...
      for(int i=0; i<size; i++)
      {
         ...
         Complex val_i = *dynamic_cast<Complex*>(m_data[i]); Complex val_j = *dynamic_cast<Complex*>(m_data[j]);
         if(bit == 0) { new_state[i] = (val_i + val_j) * 0.70710678118; new_state[j] = (val_i - val_j) * 0.70710678118; }
      }
      for(int i=0; i<size; i++) { *dynamic_cast<Complex*>(m_data[i]) = new_state[i]; }
      AddOperation("H", target); m_normalized = true; m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate H aplicado no qubit " + IntegerToString(target)); return true;
   }
   ...
   bool ApplyX(int target)
   {
      ...
      for(int i=0; i<size; i++)
      {
         int bit = (i >> target) & 1; if(bit == 1) continue;
         int j = i ^ (1 << target); Complex temp = *dynamic_cast<Complex*>(m_data[i]); *dynamic_cast<Complex*>(m_data[i]) = *dynamic_cast<Complex*>(m_data[j]); *dynamic_cast<Complex*>(m_data[j]) = temp;
      }
      AddOperation("X", target); m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate X aplicado no qubit " + IntegerToString(target)); return true;
   }
   ...
   bool ApplyZ(int target)
   {
      ...
      for(int i=0; i<size; i++)
      {
         int bit = (i >> target) & 1; if(bit == 1) { Complex *c = dynamic_cast<Complex*>(m_data[i]); c->imag = -c->imag; }
      }
      AddOperation("Z", target); m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate Z aplicado no qubit " + IntegerToString(target)); return true;
   }

   bool ApplyCNOT(int control, int target)
   {
      if(!is_valid_context() || control >= m_qubits || control < 0 || target >= m_qubits || target < 0 || control == target) return false;
      int size = (int)MathPow(2, m_qubits);
      for(int i=0; i<size; i++)
      {
         int c_bit = (i >> control) & 1; if(c_bit == 0) continue; int j = i ^ (1 << target);
         Complex temp = *dynamic_cast<Complex*>(m_data[i]); *dynamic_cast<Complex*>(m_data[i]) = *dynamic_cast<Complex*>(m_data[j]); *dynamic_cast<Complex*>(m_data[j]) = temp;
      }
      AddOperation("CNOT", target, control); m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate CNOT aplicado (control=" + IntegerToString(control) + ", target=" + IntegerToString(target) + ")"); return true;
   }
   ...
   bool ApplyRz(int target, double theta)
   {
      ...
      for(int i=0; i<size; i++)
      {
         int bit = (i >> target) & 1; Complex *c = dynamic_cast<Complex*>(m_data[i]); double phase = (bit == 0 ? -theta/2.0 : theta/2.0);
         double new_real = c->real * MathCos(phase) - c->imag * MathSin(phase);
         double new_imag = c->real * MathSin(phase) + c->imag * MathCos(phase);
         c->real = new_real; c->imag = new_imag;
      }
      AddOperation("Rz", target, -1, theta); m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate Rz aplicado no qubit " + IntegerToString(target) + " (θ=" + DoubleToString(theta, 3) + ")"); return true;
   }
   ...
   bool ApplyCRz(int control, int target, double theta)
   {
      ...
      for(int i=0; i<size; i++)
      {
         int c_bit = (i >> control) & 1; if(c_bit == 0) continue; int t_bit = (i >> target) & 1; Complex *c = dynamic_cast<Complex*>(m_data[i]); double phase = (t_bit == 0 ? -theta/2.0 : theta/2.0);
         double new_real = c->real * MathCos(phase) - c->imag * MathSin(phase);
         double new_imag = c->real * MathSin(phase) + c->imag * MathCos(phase);
         c->real = new_real; c->imag = new_imag;
      }
      AddOperation("CRz", target, control, theta); m_last_operation = TimeCurrent(); updateRegisterDisplay(); m_logger.log_info("[QREG] Gate CRz aplicado (control=" + IntegerToString(control) + ", target=" + IntegerToString(target) + ", θ=" + DoubleToString(theta, 3) + ")"); return true;
   }

   bool Normalize()
   {
      if(!is_valid_context()) return false; double norm = 0.0; for(int i=0; i<Total(); i++) { norm += dynamic_cast<Complex*>(m_data[i])->probability(); }
      if(norm == 0.0) return false; norm = MathSqrt(norm);
      for(int i=0; i<Total(); i++) { Complex *c = dynamic_cast<Complex*>(m_data[i]); c->real /= norm; c->imag /= norm; }
      m_normalized = true; m_logger.log_info("[QREG] Estado quântico normalizado"); return true;
   }

   int Measure()
   {
      if(!is_valid_context() || !m_normalized) return -1; double probabilities[]; ArrayResize(probabilities, Total()); double sum = 0.0;
      for(int i=0; i<Total(); i++) { probabilities[i] = dynamic_cast<Complex*>(m_data[i])->probability(); sum += probabilities[i]; }
      if(sum == 0.0) return -1; for(int i=0; i<ArraySize(probabilities); i++) probabilities[i] /= sum;
      double r = MathRand() / 32767.0; double cumulative = 0.0; for(int i=0; i<ArraySize(probabilities); i++) { cumulative += probabilities[i]; if(r <= cumulative) { m_logger.log_info("[QREG] Medição realizada: Estado " + IntegerToString(i) + " colapsado"); return i; } }
      return 0;
   }

   double GetStateProbability(int state_index)
   { if(state_index < 0 || state_index >= Total()) return 0.0; return dynamic_cast<Complex*>(m_data[state_index])->probability(); }
   int GetQubitCount() const { return m_qubits; }
   ...
   bool ExportOperationHistory(string file_path)
   {
      ...
      FileClose(handle); m_logger.log_info("[QREG] Histórico de operações exportado para: " + file_path); return true;
   }
};
...
public:
   ...
   {
      if(!m_logger.is_initialized() || iterations <= 0) return 0.0; double start_time = GetMicrosecondCount(); double payoff_sum = 0.0;
   ...
   {
      if(!m_logger.is_initialized() || ArraySize(returns) == 0) return; double start_time = GetMicrosecondCount(); int n_assets = ArraySize(returns);
      ...
      double sharpe = total / risk; double end_time = GetMicrosecondCount(); double execution_time = (end_time - start_time) / 1000.0;
   ...
   {
      if(!m_logger.is_initialized()) return 0; double start_time = GetMicrosecondCount(); MqlRates rates[]; CopyRates(_Symbol, PERIOD_H1, 0, 50, rates);
   ...
   {
      if(!m_logger.is_initialized() || confidence_level < 0.0 || confidence_level > 1.0) return 0.0; double start_time = GetMicrosecondCount();
      MqlRates rates[]; CopyRates(_Symbol, PERIOD_H1, 0, 1000, rates); double returns[]; ArrayResize(returns, ArraySize(rates)-1);
      for(int i=0; i<ArraySize(rates)-1; i++) { returns[i] = MathLog(rates[i].close / rates[i+1].close); }
